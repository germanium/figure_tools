...
% bars of length error.
% groupnames  - is an m-length cellstr vector of groupnames (i.e. groupnames = 
%              'group 1'; 'group 2'}).  For no groupnames, enter [] or {}
% errors      - The errors matrix is of the same form of the barvalues matrix, 
%               namely m group of n errors.
% gridstatus  - is either 'x','xy', 'y', or 'none' for no grid.
% legend_type - No legend will be shown if the legend paramter is not provided
% error_sides - If = 2 plots +/- std while 'error_sides = 1' plots just + std
% legend_type - 'axis' produces the legend along the x-axis while legend_type = 
%               'plot' produces the standard legend.  See figure for more details
%
% The following default values are used if parameters are left out or skipped by 
% using [].
...
% legend_type = 'plot';
%
% A list of handles are returned so that the user can change the properties of 
% the plot
...
if size(barvalues,1) ~= size(errors,1) || size(barvalues,2) ~= size(errors,2)
	...
	Nbars = size(barvalues, 2); % number of bars in a group
	if isempty(width)
		width = 1;
	end
	...
	if ~isempty(bw_legend) && ~strcmp(legend_type, 'axis')
		handles.legend = legend(bw_legend, 'location', 'best', 'fontsize',20);
		legend boxoff;
	else
		handles.legend = [];
	end
	...
	set(gca, 'xticklabel', groupnames, 'box', 'off', 'ticklength', [0 0], ...
        'fontsize', 20, 'xtick',1:numgroups, 'linewidth', 2,'xgrid','off',...
        'ygrid','off');
