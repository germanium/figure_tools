...
if ~isscalar(Options.ytickoffset) || ~isscalar(Options.xtickoffset) ||...
        ~isscalar(Options.ztickoffset)
...
texttick(ax, xformatfun, yformatfun, zformatfun, Options.ytickoffset, ...
    Options.xtickoffset, Options.ztickoffset, Options.axis);

% Add customized zoom
    
try
    hzoom = zoom(ax);
    set(hzoom, 'ActionPostCallback', {@newzoompan, ax, xformatfun, yformatfun, ...
        zformatfun, Options.ytickoffset, Options.xtickoffset, Options.ztickoffset, ...
        Options.axis});
catch
...
if Options.panon
    hpan = pan(fig);
    set(hpan, 'ActionPostCallback', {@newzoompan, ax, xformatfun, yformatfun, ...
        zformatfun, Options.ytickoffset, Options.xtickoffset, Options.ztickoffset, ...
        Options.axis});
end
...
function texttick(haxis, xformat, yformat, zformat, ytickoffset, xtickoffset, ...
    ztickoffset, tickax)

% Get current values of ticks and axis limits

temp = get(haxis, {'XTick', 'YTick', 'ZTick', 'XLim', 'YLim', 'ZLim', 'XScale', ...
    'YScale', 'ZScale'});
...
if ~isempty(strfind(tickax, 'x'))
    ...
    if isappdata(haxis, 'XTickText')
        ...
        [tf,loc] = ismember({'Extent', 'String', 'BeingDeleted', 'Children', ...
        ...
        hx = text(xtick, xlabely, xlabelz, xticklab, 'horizontalalignment', ...
            'center', 'parent', haxis);      
        setappdata(haxis, 'XTickText', hx);
    end
end
...
if ~isempty(strfind(tickax, 'y'))
    ...
    if isappdata(haxis, 'YTickText')
        ...
        hy = text(ylabelx, ytick, ylabelz, yticklab, 'horizontalalignment', ...
            'center', 'parent', haxis);
        setappdata(haxis, 'YTickText', hy);
    end
end
...
if ~isempty(strfind(tickax, 'z'))
    ...
    if isappdata(haxis, 'ZTickText')
        ...
        hz = text(zlabelx, zlabely, ztick, zticklab, 'horizontalalignment', ...
            'center', 'parent', haxis);
        setappdata(haxis, 'ZTickText', hz);
    end
end
